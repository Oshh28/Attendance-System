...
function toggleDropdown(event, studentId) {
    ...
    // Step 1: Place off-screen first so we can measure its real height
    ...
    // Step 2: Now measure real dimensions
    const dropWidth  = dropdown.offsetWidth  || 160;
    ...
    // Default: show below button, right-aligned to button
    let top  = rect.bottom + 4;
    let left = rect.right - dropWidth;

    // Not enough space below? → show ABOVE the button
    if (top + dropHeight > vh - 8) {
        top = rect.top - dropHeight - 4;
    }

    // Off left edge? → align to button left
    if (left < 8) left = rect.left;

    // Off right edge? → push left
