...
async function submitGatePass() {
    ...
    try {
        ...
        // Show success modal — Time In or Time Out
        const studentName = document.getElementById('previewName').textContent || '—';
        const attendance  = data.attendance || 'Present';
        const scanType    = data.scan_type  || 'TIME_IN';
        const timeValue   = scanType === 'TIME_OUT'
                          ? (data.time_out || timeIn)
                          : (data.time_in  || timeIn);
        showSuccess(studentNo, studentName, section, timeValue, attendance, scanType);
...
//   SHOW SUCCESS MODAL
// =====================================================
function showSuccess(studentNo, studentName, section, timeValue, attendance = 'Present', scanType = 'TIME_IN') {
    const isTimeOut = scanType === 'TIME_OUT';

    // Color based on scan type or attendance
    const attColor = isTimeOut            ? '#1565c0'
                   : attendance === 'Present' ? '#16a34a'
                   : attendance === 'Late'    ? '#d97706'
                   : '#dc2626';

    const attLabel = isTimeOut ? 'Time Out' : attendance;
    const timeLabel = isTimeOut ? 'Time Out' : 'Time In';
    const msg = isTimeOut
        ? 'Time Out has been successfully recorded.'
        : 'Time In has been successfully recorded.';

    document.getElementById('gpSuccessMsg').textContent = msg;
    document.getElementById('gpSuccessDetails').innerHTML = `
        ...
        <strong>${timeLabel}:</strong> ${timeValue}<br>
        <strong>Status:</strong> <span style="font-weight:800;color:${attColor};">${attLabel}</span><br>
...
async function loadTodayLogs() {
    try {
        ...
        tbody.innerHTML = data.logs.map((log, i) => {
            ...
            const outStyle = log.time_out ? 'color:#1565c0;font-weight:600;' : 'color:#bbb;';
            return `
            <tr>
                <td>${i + 1}</td>
